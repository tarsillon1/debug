...
xmppServer.prototype.setup = function(s2sPort, bindAddress, domain, opts) {
  ...
  var server = this;
  this.router.loadCredentialsFromFile(
    domain,
    opts.tls.keyPath,
    opts.tls.certPath
  );
...
var parsePushStanza = function(stanza, cb) {
  console.log("handle the push stanza man", status);
  ...
  var token = null;
  if (
    typeof publishOptionsStanza !== "undefined" &&
    publishOptionsStanza !== null
  ) {
    var publishOptionsFormData = publishOptionsStanza.getChildrenByFilter(
      function(child) {
        return child.name === "x" && child.attrs.xmlns === "jabber:x:data";
      },
      true
...
xmppServer.prototype.emitPushEvent = function(pushInfo) {
  console.log("handle the push man", pushInfo);
...
xmppServer.prototype.handleStanza = function(stanza) {
  console.log("handle the stanza man", status);
